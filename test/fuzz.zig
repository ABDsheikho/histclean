...
test "fuzz: filterLines with arbitrary bytes" {
    inline for (std.meta.tags(histclean.ShellEnum)) |shell| {
        ...
        while (i < 200) : (i += 1) {
            ...
            const result = histclean.filterLines(shell, input, testing.allocator) catch continue;
...
test "fuzz: filterLines results are deterministic" {
    inline for (std.meta.tags(histclean.ShellEnum)) |shell| {
        ...
        while (i < 100) : (i += 1) {
            ...
            const r1 = try histclean.filterLines(shell, input, testing.allocator);
            defer testing.allocator.free(r1);

            const r2 = try histclean.filterLines(shell, input, testing.allocator);
